 ...
 */
export function rollDrops(rng: Rng, type: EnemyType): DropRoll {
  ...
  return {
    ...
    consumable: consumableDraw < CONSUMABLE_CHANCE,
  };
}
...
export interface DropPlan {
  ...
  credit: number;
  /** Whether to place the consumable. One with no room is lost. */
  consumable: boolean;
}
...
export function planDrop(roll: DropRoll, live: number, cap: number = MAX_LIVE_PICKUPS): DropPlan {
  ...
  if (ember > 0) room -= 1;
  return { ember, credit: roll.embers - ember, consumable: roll.consumable && room > 0 };
}
